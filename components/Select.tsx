export default function Select({ label, data, setData, name, disabled }: { label: string; data: string[] | { name: string; characteristics: string }[]; setData: (data: any) => void; name: string; disabled?: boolean }) {
	...
	return (
		<div className="flex flex-col items-start w-full">
			...
			<select className={`${disabled ? "cursor-not-allowed bg-gray-100" : "cursor-pointer bg-white"} appearance-none md:appearance-auto bg-white border border-gray-200 text-gray-900 text-sm rounded-lg block w-full p-2.5`} onChange={handleChange} disabled={disabled}>
